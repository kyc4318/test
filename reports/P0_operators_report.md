...
> （见本报告 §8 待办）。
>
> **仍然站得住的相关证据**：(i) 旋转格上 **null 角度泄漏率仅 0.02–0.04**
> （`P0_controls_report.md` §5）——无水印图被同样旋转时估计角并不跟随真实角，
> 该检验不依赖任何填充算子的几何假设；(ii) `cv2_linear_reflect`（反射填充，
> 无黑边）在 canonical 解码下仍为 BitAcc 0.977 / PMR 0.900 / 失锁 0.033，
> 属于「换掉黑边仍然可用」的一个数据点，但需要在配对实验中确认。
