...
> ### 最终读数（2026-09-21，含配对统计；本节取代下面所有旧表述）
>
> **1. 算子泛化：通过（但要用配对差说）。**
...
> * **真正决定性的差异来自几何消融**：把解码端旋转中心挪到 DFT 原点（`c=32`）后，
>   `cv2_linear_reflect` 格 ΔBitAcc **−0.135 [−0.190, −0.085]**、
>   Δ失锁率 **+0.250 [+0.150, +0.350]**（15/60 对不一致），
>   其余 `*_c32` 格也都是 ΔBitAcc −0.058…−0.065、Δ失锁率 +0.100 [0.033, 0.183]，
>   **区间不含 0**。
>
> **2. 因此报告结构应当是两段，而不是一个总失锁率**：
> **canonical 方法**（物理中心 31.5）与**几何消融**（故意用 DFT 原点 32）。
> 把两者混进一个「overall 3.92%」是不恰当的——那 47/1200 里有一半来自
> 一个**故意设错的配置**。
>
> **3. 黑边问题：干净的因果消融**还没跑**。** 实际那次运行的 `--attack_ops`
> 是 `pil_bilinear, pil_bicubic, pil_expand_crop, cv2_linear_reflect,
> cv2_cubic_constant`——**既没有 `cv2_linear_constant` 也没有
> `cv2_cubic_reflect`**，所以三对「同库/同插值/仅边界不同」的配对**全部缺一半**
> （已核对 `rows.jsonl` 的算子清单）。因此不能声称「换掉黑边无退化」。
>
> 现有数据能给的是**跨实现比较**（不同库 + 不同边界 + cubic/linear 不同）：
> `cv2_linear_reflect` 相对 `pil_bilinear` 参考格 ΔBitAcc **−0.019
> [−0.048, +0.000]**（nearest 解码）/ **−0.031 [−0.088, +0.000]**（bilinear 解码），
> 区间**触到 0** → 未达可分辨退化，但这不是边界规则的隔离实验。
>
> 与填充无关的正面证据仍然是**角度锁定检验**（无水印图在同一算子下只 1–4%
> 锁定真角，水印图 100/100），据此可以写「**未观察到由攻击算子伪影单独驱动的
> 角度锁定**」；「黑边与载波交互」这一较弱解释**尚未排除**。
> 三对匹配实验已写入 `run_p0_session.sh` 的默认 `--attack_ops`，下一次 GPU 运行
> 即可闭环。
>
> **4. 规模**：19 个 cell 里最差的判别力来自「不一致对数」只有 2–6 对，
> 因此上面的「未观察到差异」是**在当前 15 图 × 4 角**下的结论。

**（以下为初版文字，保留以对照；其中 `pil_expand_crop` 的解读已作废。）**
...
| `pil_expand_crop`（⚠️**不是去黑边对照**，与 `pil_bilinear` 逐像素相同） | 0.000 / 0.9958 (0.97) | 0.000 / 0.9958 (0.97) | 0.000 / 0.9979 (0.98) | 0.033 / 0.9313 (0.73) |
